        ...
        <div class="container mx-auto px-4 py-8">
            <div v-if="items.length > 0" class="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
                <div v-for="item in items" :key="item.id"
                    class="group bg-theme-card rounded-2xl shadow-theme-lg overflow-hidden hover:-translate-y-1 transition-all duration-300">
                    <div class="relative aspect-square overflow-hidden">
                        <img :src="item.image" :alt="item.name"
                            class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                        <button @click="removeItem(item.id)"
                            class="absolute top-3 right-3 w-8 h-8 bg-theme-card rounded-full flex items-center justify-center shadow-lg text-red-500 hover:bg-red-500 hover:text-white transition">
                    ...
                    <div class="p-4">
                        <p class="text-xs text-primary mb-1">{{ item.category }}</p>
                        <h4
                            class="font-medium text-theme mb-2 line-clamp-2 group-hover:text-primary transition">
                            {{ item.name }}</h4>
                        <div class="flex items-center justify-between">
                            <span class="text-xl font-bold text-theme">${{ item.price }}</span>
                            <button
                                class="px-4 py-2 bg-primary hover:bg-accent text-white rounded-lg font-medium transition text-sm">
                                Add to Cart
                            </button>
                        </div>
                    </div>
            ...
            <div v-else class="text-center py-20">
                ...
                <h2 class="text-xl font-semibold text-theme mb-2">Your wishlist is empty</h2>
                <p class="text-theme-muted mb-6">Save items you love for later</p>
                <NuxtLink to="/shop"
